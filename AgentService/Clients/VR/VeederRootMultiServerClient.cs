    ...
    public class VeederRootMultiServerClient : AMultiServerClient {
            ...
            try {
                ...
                foreach (var source in ObjectSettings.Objects.First().ObjectSources) {
                    if (int.TryParse(source.InternalId, out int nom)) {
                        if (Send231(nom, ObjectSettings.IpConnectionConfig.DComPwd) <= 0) {
                    ...
                    if (Send215(nom, ObjectSettings.IpConnectionConfig.DComPwd) <= 0) {
        ...
        private DateTime? fillDTVR(int nom)
        {
            ...
            try {
                ...
                Logger.Error($"error on fillDTVR {e.Message + e.StackTrace}. Year {yy}, month {mm}, day {dd}, hours {hh}, minutes {mn}");
                return null;
            }
        }
        ...
        {
            var result = bytes.Aggregate("", (current, b) => current + (" " + b.ToString("X2")));
            return result.Trim();
        }
