...
namespace Service.Clients.PV4 {
    public class Pv4MultiServerClient : AMultiServerClient {
        ...
        private static DateTime? fillDT(byte[] bytes)
        {
            try {
                ...
                if (DateTime.TryParseExact(str, "MMddyyHHmm", CultureInfo.InvariantCulture, DateTimeStyles.AdjustToUniversal, out result))
                    return result;
        ...
        private bool FilllSSVersion()
        {
            ...
            Logger.Info("System version = {0}", sysVersion);
            Logger.Info("Country = {0}", country);
            Logger.Info("Board number = {0}", brdNumber);
            Logger.Info("Site Sentinel type = {0}", ssType);
            Logger.Info("Actual DateTime = {0}", actualDateTime);
            ...
            try {
                ...
                for (var i = 0; i < b; i++) {
                    ...
                    if (st != TankInventoryTdStatus.TankOk) {
                        Logger.Info("TankNotFound {0} ", tanknum);
                        pos += 34;
                        continue;
                    }
        ...
        private List<TankMeasurementDataPv4Dto> FillTankLevelsExt()
        {
            ...
            try {
                ...
                for (var i = 0; i < b; i++) {
                    ...
                    if (st != TankInventoryTdStatus.TankOk) {
                        Logger.Info("TankNotFound {0} Status {1}", nn, st);
                        pos += 47;
                        continue;
                    }
        ...
        private List<TankTransferData> FillDeliveryEnquiry2()
        {
            ...
            try {
                ...
                if (stuff.s != DeliveryStatus.Stable) {
                    Logger.Info("Tank {0} Delivery TankStatus is {1}", nn, stuff.s);
                    return null;
                }
        ...
        private List<TankTransferData> FillDeliveryEnquiryExt2()
        {
            ...
            try {
                ...
                if (stuff.s != DeliveryStatus.Stable) {
                    Logger.Info("Tank {0} Delivery TankStatus is {1}", tanknum, stuff.s);
                    return null;
                }
